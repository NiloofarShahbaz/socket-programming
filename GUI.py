...
class Window(Frame):
    ...
    def init_window(self, client_fun):
        ...
        # label.pack()
        quitButton1 = Button(self, text="Connect to server", command=client_fun, padx=10, pady=15, bg="royalblue3",
    ...
    def getting_request(self):
        label = Label(self, text="please wait for the other client to send request", font=self.server_font,
                      fg='royalblue3')
        label.place(x=70, y=150)

    def alert_request(self, msg, client):

        ans = messagebox.askquestion("Request Information",
                                     "client " + str(msg.get('from')) + " wants to send an audio with name " +
                                     str(msg.get('audio_name')) + " and size " + str(
                                         msg.get('audio_size')) + " ,Do you want to recive it?")
